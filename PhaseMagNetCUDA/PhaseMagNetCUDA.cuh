...
private:
	...
	bool initialized;
	void setInput(const uchar* const ucharptr, float linfNoise = 0.0f, float gaussNoise = 0.0f);
...
public:
	...
	void free(void);
	void train(const unsigned int num_examples, uchar** inputData, uchar* labels, float lrnRate = 0.01, bool verbose = false, float dropout = 0.0f, float linfNoise = 0.0f, float gaussNoise = 0.0f);
	...
	Matrix<DTYPE> predict(const unsigned int num_examples, uchar** inputData, bool verbose = false, float linfNoise = 0.0f);
	float evaluate(const unsigned int num_examples, uchar** inputData, uchar* labels, bool verbose = false, float linfNoise = 0.0f);
